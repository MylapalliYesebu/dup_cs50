...
// Function to calculate and print the checksum
void printCheckSum(int b1[], int b2[], int n) {
    ...
    for(i = n-1; i >= 0; i--) {
        t = b1[i] + b2[i] + carry;
        temp[i] = t % 2;          // Store the sum bit
        carry = t / 2;            // Calculate carry
    }

    // If there is a carry left after the addition, add it to the result
    while(carry) {
        for(i = n-1; i >= 0; i--) {
            t = temp[i] + carry;
            temp[i] = t % 2;
    ...
    // Calculate and print the checksum (1's complement of the sum)
    printf("Checksum: ");
    for(i = 0; i < n; i++)
        printf("%d", !temp[i]);
    printf("\n");
