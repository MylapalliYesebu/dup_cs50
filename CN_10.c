...
void dijkstra(int graph[V][V], int src) {
    ...
    for (int count = 0; count < V - 1; count++) {
        ...
        for (int v = 0; v < V; v++) {
            // Update dist[v] if and only if it is not in sptSet, there is an edge from u to v, and the total weight of path from src to v through u is smaller than the current value of dist[v]
